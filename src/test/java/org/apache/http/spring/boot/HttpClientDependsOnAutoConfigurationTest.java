...
/**
 * Unit tests for {{ @link HttpClientDependsOnAutoConfiguration }}.
 *
...
class HttpClientDependsOnAutoConfigurationTest {
    ...
    void testInstantiation() {
        HttpClientDependsOnAutoConfiguration configuration = new HttpClientDependsOnAutoConfiguration();
    ...
    void testLoadsWhenEnabledPropertySet() {
        runner.withUserConfiguration(HttpClientDependsOnAutoConfiguration.class)
                .withPropertyValues("spring.boot.enabled=true")
                .run(context -> assertThat(context).hasSingleBean(HttpClientDependsOnAutoConfiguration.class));
    }
    ...
    void testNotLoadedWhenPropertyAbsent() {
        runner.withUserConfiguration(HttpClientDependsOnAutoConfiguration.class)
                .run(context -> assertThat(context).doesNotHaveBean(HttpClientDependsOnAutoConfiguration.class));
    }
}
