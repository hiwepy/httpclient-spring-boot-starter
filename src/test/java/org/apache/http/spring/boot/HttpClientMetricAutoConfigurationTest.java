...
/**
 * Unit tests for {{ @link HttpClientMetricAutoConfiguration }}.
 *
...
class HttpClientMetricAutoConfigurationTest {
    ...
    void testInstantiation() {
        HttpClientMetricAutoConfiguration configuration = new HttpClientMetricAutoConfiguration();
    ...
    void testLoadsWhenEnabledPropertySet() {
        runner.withUserConfiguration(HttpClientMetricAutoConfiguration.class)
                .withPropertyValues("spring.boot.enabled=true")
                .run(context -> assertThat(context).hasSingleBean(HttpClientMetricAutoConfiguration.class));
    }
    ...
    void testNotLoadedWhenPropertyAbsent() {
        runner.withUserConfiguration(HttpClientMetricAutoConfiguration.class)
                .run(context -> assertThat(context).doesNotHaveBean(HttpClientMetricAutoConfiguration.class));
    }
}
